...
public class IdempotencyFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (existing.isPresent()) {
            ...
            log.info("Idempotent tekrar istek tespit edildi, cached response dönülüyor: key={}", idempotencyKey);
        ...
        if (status >= 200 && status < 300) {
            saveIdempotencyRecord(idempotencyKey, status, wrappedResponse);
        }
