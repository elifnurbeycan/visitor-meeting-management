...
public class IdempotencyFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        Optional<IdempotencyKey> existing = idempotencyKeyRepository
                .findByIdempotencyKeyAndCompanyIdAndUserId(idempotencyKey, companyId, userId);

        if (existing.isPresent()) {
            IdempotencyKey cached = existing.get();
            log.info("Idempotent tekrar istek tespit edildi, cached response dönülüyor: key={}, userId={}", idempotencyKey, userId);
        ...
        if (status >= 200 && status < 300) {
            saveIdempotencyRecord(idempotencyKey, companyId, userId, status, wrappedResponse);
        }
    ...
    protected void saveIdempotencyRecord(
            String idempotencyKey, Long companyId, Long userId, int status, ContentCachingResponseWrapper wrappedResponse
    ) {
        ...
        log.debug("Idempotency key kaydedildi: {}, userId={}", idempotencyKey, userId);
    }
